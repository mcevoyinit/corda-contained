...
setInitialNetworkParameters () {
	java -Dlog4j.configurationFile=file:./log4j2.xml -jar doorman.jar --config-file ./persistent/network-map.conf --set-network-parameters ./persistent/network-parameters.conf --network-truststore ./persistent/certificateStore.jks --truststore-password example-password --root-alias cordarootca --ignore-migration
	checkStatus $?
...
startTheNetmap () {
	java -Dlog4j.configurationFile=file:./log4j2.xml -jar doorman.jar --config-file ./persistent/network-map.conf
	checkStatus $?
}
